...
class RecoveryAttemptRepository(
    ...
    fun claimDueAttempts(limit: Int, leaseDuration: Duration): List<RecoveryAttempt> {
        return measure("claim_due_attempts") {
            val now = OffsetDateTime.now()
    ...
    fun markDispatched(attemptId: String): Boolean {
        return measure("mark_dispatched") {
            dataSource.connection.use { connection ->
                connection.prepareStatement(
                    """
    ...
    fun claimExecution(attemptId: String, leaseDuration: Duration): RecoveryAttempt? {
        return measure("claim_execution") {
            val now = OffsetDateTime.now()
            val leaseUntil = now.plus(leaseDuration)
            dataSource.connection.use { connection ->
                connection.autoCommit = false
    ...
    fun findByCartId(cartId: String): List<RecoveryAttempt> {
        return measure("find_by_cart_id") {
            dataSource.connection.use { connection ->
                connection.prepareStatement(
                    """
    ...
    fun findByAttemptId(attemptId: String): RecoveryAttempt? {
        return measure("find_by_attempt_id") {
            dataSource.connection.use { connection ->
                connection.prepareStatement(
                    """
                    select attempt_id, cart_id, tenant_id, user_id, experiment_id, experiment_name,
    ...
    fun updateExecutionOutcome(
        ...
        return measure("update_execution_outcome") {
            dataSource.connection.use { connection ->
                connection.prepareStatement(
                    """
                    update recovery_attempt
    ...
    private inline fun <T> measure(operation: String, block: () -> T): T {
        ...
        try {
            ...
            recoveryMetrics.recordRepositoryOperation(
                repository = "recovery_attempt",
                operation = operation,
                outcome = outcome,
